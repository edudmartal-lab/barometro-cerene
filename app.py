...
def detect_columns(df):
    ...
    col_adult = next(
        (
            c
            for c in columns
            if any(k in normalize_text(c) for k in adult_keywords)
        ),
        None,
    )
    col_date = next(
...
def render_observation_cards(data, cols, title, severe_only=False, limit=5):
    ...
    for _, row in view.iterrows():
        ...
        if cols.get("adult") and cols["adult"] in view.columns and pd.notna(row.get(cols["adult"])):
            adult_value = str(row.get(cols["adult"]))
        ...
        st.markdown(
            ...
            <div class='obs-card{severe_cls}' style='border-left-color:{palette['border']};'>
                ...
                <div class='obs-main'><b>{eleve}</b> · {classe}</div>
                {f"<div class='obs-adult'>👤 {adult_value}</div>" if adult_value else ""}
...
def render_dashboard(df, df_f, cols):
    ...
    with t7:
        ...
        st.caption("Distribution des observations par adulte/professeur sur les filtres actifs.")

        if col_adult and col_adult in df.columns:
            df_team = df_f[df_f[col_adult].notna()].copy()
            if df_team.empty:
                st.info("Aucune donnée équipe disponible pour les filtres sélectionnés.")
            else:
                c1, c2 = st.columns([1, 3])
                with c1:
                    profs = sorted(df_team[col_adult].astype(str).unique())
                    sel_p = st.selectbox("Professeur", ["Global"] + profs)
                with c2:
                    if sel_p == "Global":
                        fig = px.histogram(
                            df_team,
                        ...
                        sub = df_team[df_team[col_adult] == sel_p]
                        fig = px.histogram(
                            sub,
                            x="Jour_Fr",
                            color="Couleur_Clean",
            ...
            st.info("Aucune colonne adulte/professeur détectée dans la source Google Sheets.")
